...
const getRouter = (basePath: string = '/', routePath: string = '/widgets') => {
...
const getAdminRouter = (basePath: string = '/', routePath: string = '/management/widgets') => {
  const router: Router = Router();
  const path = urljoin(basePath, routePath);
  ...
  router.post(
    ...
    validate([
      ...
      body('metrics.*').isString().trim().notEmpty(),
      body('layers').isArray(),
      body('layers.*').isString().trim().notEmpty(),
      query('group').optional().isString().trim(),
    ]),
    guard.enforcePrimaryGroup(true),
  ...
  router.delete(
    `${path}/:id`,
    validate([param('id').isString(), query('group').optional().isString().trim()]),
    guard.enforcePrimaryGroup(),
    AuthzGuards.writeWidgetsGuard,
    asyncHandler(async (req: AuthzRequest, res: Response) => {
      const id = req.params.id;
